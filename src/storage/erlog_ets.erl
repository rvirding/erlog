...
assert(Db) ->
	foldl(fun({Head, M, F}, LDb) ->
		erlog_int:add_compiled_proc(Head, M, F, LDb) end, Db,
		[
...
all_1({ets_all, Var}, Next, Cps, Bs, Vn, Db) ->
	Tabs = ets:all(),
	erlog_int:unify_prove_body(Var, Tabs, Next, Cps, Bs, Vn, Db).
...
keys_2({ets_keys, Tab0, KeyVar}, Next, Cps, Bs, Vn, Db) ->
	Tab1 = erlog_int:dderef(Tab0, Bs),
	case ets:first(Tab1) of
		'$end_of_table' -> erlog_int:fail(Cps, Db);
...
keys_loop(Tab, Key, KeyVar, Next, Cps, Bs, Vn, Db) ->
	...
	erlog_int:unify_prove_body(KeyVar, Key, Next, [C | Cps], Bs, Vn, Db).
...
match_2_loop({[M | Ms], Cont}, Next, Cps, Bs, Vn, Db, Epat, Vs) ->
	...
	erlog_int:unify_prove_body(Vs, M, Next, [Cp | Cps], Bs, Vn, Db);
