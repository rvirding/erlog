...
-export([type_error/3, instantiation_error/1, permission_error/4,
	type_error/2, instantiation_error/0, erlog_error/2, erlog_error/1, fail/2]).
...
fail_disjunction(#cp{next = Next, bs = Bs, vn = Vn}, Cps, Db) ->
	erlog_core:prove_body(Next, Cps, Bs, Vn, Db).
...
fail_ecall(#cp{data = {Efun, Val}, next = Next, bs = Bs, vn = Vn}, Cps, Db) ->
	erlog_core:prove_ecall(Efun, Val, Next, Cps, Bs, Vn, Db).

%% @private
fail_clause(#cp{data = {Ch, Cb, Cs}, next = Next, bs = Bs, vn = Vn}, Cps, Db) ->
	erlog_core:unify_clauses(Ch, Cb, Cs, Next, Cps, Bs, Vn, Db).

%% @private
fail_retract(#cp{data = {Ch, Cb, Cs}, next = Next, bs = Bs, vn = Vn}, Cps, Db) ->
	erlog_core:retract_clauses(Ch, Cb, Cs, Next, Cps, Bs, Vn, Db).

%% @private
fail_current_predicate(#cp{data = {Pi, Fs}, next = Next, bs = Bs, vn = Vn}, Cps, Db) ->
	erlog_core:prove_predicates(Pi, Fs, Next, Cps, Bs, Vn, Db).

%% @private
fail_goal_clauses(#cp{data = {G, Cs}, next = Next, bs = Bs, vn = Vn}, Cps, Db) ->
	erlog_core:prove_goal_clauses(G, Cs, Next, Cps, Bs, Vn, Db).