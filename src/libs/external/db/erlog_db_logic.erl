...
retractall_clauses(Table, Clause, H, B, Params = #param{bindings = Bs0, var_num = Vn0, database = Db, cursor = Cursor}) ->
  case erlog_ec_unify:unify_clause(H, B, Clause, Bs0, Vn0) of
    {succeed, _, _} ->
      {{UCursor, Res}, UDb} = erlog_memory:db_retract_clause(Db, Table, erlog_ec_support:functor(H), element(1, Clause)),
      {{UCursor, Res}, UDb2} = erlog_memory:db_next(UDb, Cursor, Table),
