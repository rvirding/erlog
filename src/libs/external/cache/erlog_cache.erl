...
load(DbState) ->
  case get(erlog_cache) of
    undefined ->
      Ets = ets:new(erlog_cache, []), %TODO use smth else instead of ets.
      put(erlog_cache, Ets);
    _ -> ok
  end,
  lists:foldl(fun(Proc, UDBState) -> erlog_memory:load_native_library(UDBState, Proc) end, DbState, ?ERLOG_CACHE).
