...
load(Db) ->
	lists:foreach(fun(Proc) -> erlog_memory:load_kernel_space(Db, ?MODULE, Proc) end, ?ERLOG_LISTS),
	%% Finally interpreted common list library.
	lists:foreach(fun(Clause) -> erlog_memory:assertz_clause(Db, Clause) end, %TODO change me to kernelspace
		[
			%% perm([], []).
...
prove_goal(Params = #param{goal = {append, A1, L, A3}, next_goal = Next0, bindings = Bs0, choice = Cps,
	...
	case ec_support:deref(A1, Bs0) of
		...
		{_} = Var ->        %This can backtrack
			FailFun = fun(LCp, LCps, LDb) ->  %TODO db not needed
				el_logic:fail_append(LCp, Params#param{choice = LCps, database = LDb, f_consulter = Fcon}, Var, L, A3)
			end,
...
prove_goal(Params = #param{goal = {sort, L0, S}, bindings = Bs}) ->
	...
	ec_body:unify_prove_body(S, L1, Params);
%% reverse([], []).
...
prove_goal(Params = #param{goal = {reverse, A1, A2}}) ->
	el_logic:reverse({reverse, A1, A2}, Params).